...
### 文案硬规则（0829 加入——教训：只凭转述干活会踩雷，必须读原文）

凡新增/修改**任何用户可见文字**（copy_zh.json、Mock 数据里的展示文案、洞察卡内容都算），动笔前：

1. **必须先完整读这两份原文**，不许凭记忆或别人的转述：
   - `~/Desktop/coding/12-女性身体运行时/01-她律/07-AI创建的放这里/01-AI行为规则.md`
   - `~/Desktop/coding/12-女性身体运行时/01-她律/07-AI创建的放这里/05-文案调性与语言风格.md`
2. **信息分层 L1-L4**：用户第一眼只许看到一句人话结论（带比喻、带"不是你的错"、低谷必配可做的小事）；数字、术语、样本量、证据等级一律放下钻层。"HRV 43.9 vs 48.2" 这类对比数字出现在第一层 = 违规
3. **出稿过五关**（01-J 节：依据/与我何干/然后呢 · 数字翻译成她的日子 · 删除测试 · 废话测试 · 杠杆律），任何一关不过就重写
4. 禁止：报告腔括号数字、"效应量/可行动性最强"类分析标签、跟人群比、医学结论、无解法的坏消息
5. 拿不准就问 Ginger，不要自行发挥
